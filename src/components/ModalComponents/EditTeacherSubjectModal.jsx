...
export default function EditTeacherSubjectModal({
  isOpen,
  onClose,
  teacherId,
  teacherData,
  ...
  // Populate form with current subject data when modal opens
  useEffect(() => {
    if (!isOpen || !subjectToEdit) return;

    setFormData({
      subject_id: subjectToEdit.subject_id?._id || "",
      day: subjectToEdit.day || "",
      room: subjectToEdit.room || "",
      time: subjectToEdit.time || "",
      slots: subjectToEdit.slots || 0,
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Find the index of the subject being edited
      const subjectIndex = teacherData.subjects.findIndex(
        (s) =>
          (s._id || s.subject_id._id) ===
          (subjectToEdit._id || subjectToEdit.subject_id._id)
      );
      ...
      // Create updated subject entry
      const updatedSubject = {
        ...teacherData.subjects[subjectIndex],
      ...
      // Create new subjects array with the updated subject
      const updatedSubjects = [...teacherData.subjects];
      updatedSubjects[subjectIndex] = updatedSubject;

      // Update teacher record with modified subjects array
      ...
      console.error("Failed to update subject assignment:", err);
      setErrorMsg(
        err.response?.data?.error || "Failed to update subject assignment."
      );
    } finally {
      setSubmitting(false);
    }
